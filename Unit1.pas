...
begin
  // Obtém o índice do caractere da última palavra no texto.
  Texto := RichEdit.Text;
  UltimaPalavra := LastDelimiter(' ', Texto);

  if UltimaPalavra > 0 then
  begin
    // Obtém o índice do caractere que inicia a palavra anterior.
    PosicaoAnterior := LastDelimiter(' ', Copy(Texto, 1, UltimaPalavra-1)) + 1;

    // Define a seleção para incluir a última palavra e a exclui.
...
begin
  ...
  try
    // Define o título da janela do diálogo de salvar.
    SaveDialog.Title := 'Salvar arquivo';

    // Define as extensões permitidas e o tipo de arquivo padrão.
    SaveDialog.Filter := 'Arquivos RTF (*.rtf)|*.rtf|Todos os arquivos (*.*)|*.*';
    SaveDialog.DefaultExt := 'rtf';

    // Exibe o diálogo de salvar e verifica se o usuário pressionou o botão OK.
    if SaveDialog.Execute then
    begin
      // Salva o conteúdo do TRichEdit no arquivo selecionado.
